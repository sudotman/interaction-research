...
namespace Oculus.Interaction
{
    public class InteractableColorVisual : MonoBehaviour
    {
        ...
        private Coroutine _routineForReset = null;
        ...
        [SerializeField]
        bool customMonitor = false;

        bool flashResetDone = false;
        ...
        public void FlashYellow()
        {
            ...
            _normalColorState = new ColorState() { Color = Color.yellow };
            _selectColorState = new ColorState() { Color = Color.yellow };
        ...
        public void FlashGreen()
        {
            ...
            _normalColorState = new ColorState() { Color = Color.green };
            _selectColorState = new ColorState() { Color = Color.green };
        ...
        protected virtual void UpdateVisual()
        {
            ...
            if (customMonitor)
            {
                ...
                {
                    InteractableIndividual tempIndividual = GetComponentInParent<InteractableIndividual>();

                    if (tempIndividual.typeOfInteractable == InteractableType.Poke)
                    {
                        tempIndividual.PlayAudioControl(1);
                    }

                    if (!tempIndividual.currentlyActive)
                    {
                        Debug.LogError("Wrong Selected");
                        _normalColorState = new ColorState() { Color = Color.red };
                        _selectColorState = new ColorState() { Color = Color.red };

                        GetComponentInParent<ResearchManager>().ResetStreak();

                        if (_routineForReset != null)
                            StopCoroutine(_routineForReset);
                        
                        _routineForReset = StartCoroutine(ResettingBackCoroutine(tempIndividual));

                        if (!flashResetDone)
                        {
                            UpdateVisualWithoutRecursion();
                            //InteractableView.WhenStateChanged += UpdateVisualState;
                            flashResetDone = true;
                        }
            ...
            if (objectToHover != null)
            {
                if (InteractableView.State == InteractableState.Hover)
                {
                    ...
                    objectToHover.SetActive(true);
                }
                else
                {
                    objectToHover.SetActive(false);
                }
            }
             
        }
        
        public void ResetBackOutsideCall()
        {
            StartCoroutine(ResettingBackCoroutine(GetComponentInParent<InteractableIndividual>()));
        }

        IEnumerator ResettingBackCoroutine(InteractableIndividual iv)
        {
            yield return new WaitForSeconds(1.5f);
            _normalColorState = new ColorState() { Color = Color.black };
            _selectColorState = new ColorState() { Color = Color.black };

            iv.ResetToOriginalPosition();

            UpdateVisual();
        }

        protected virtual void UpdateVisualWithoutRecursion()
        {
            ColorState target = ColorForState(InteractableState.Select);
            CancelRoutine();
            _routine = StartCoroutine(ChangeColor(target));
            
        }

        public virtual void UpdateVisualManual()
        {
            ColorState target = ColorForState(InteractableState.Normal);
            if (target != _target)
            {
                _target = target;
                CancelRoutine();
